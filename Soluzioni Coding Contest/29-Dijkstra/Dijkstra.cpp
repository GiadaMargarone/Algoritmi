...
private:
    ...
    void Heapify(int i)
    {
        ...
        int l = _getLeft(i);
        int min = i;
        if (l < p && c[l].d < c[min].d)
            min = l;
        if (r < p && c[r].d < c[min].d)
            min = r;
        if (min != i)
        {
            Coppie<H> tmp = c[i];
...
template <class H>
int Dijkstra(Graph<H> *g, H s, H dest, Coppie<H> *d, int V)
{
    int dist = 0;
    Heap<H> *h = new Heap<H>(V);
    for (int i = 0; i < V; i++)
        h->Insert(g->nodevalue(i));
    int i = 0;
    h->HeapDecrease(s, 0);
    while (h->getSize() > 0)
    {
